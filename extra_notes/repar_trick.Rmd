...
and apply gradient descent with respect to $\beta$ instead. As $\beta$ is in the range of $R$. By doing this, gradient descent will usually perform more stably without the danger of stepping into those undefined regine of $\theta$. When it converges, we can apply the same transform to find the corresponding $\theta \leftarrow f(\beta)$\footnote{$\leftarrow$ means assignment.}. 
...
To simply the problem, let us assume $\mu$ is known. The only unknown parameter is $\sigma^2$, the variance. Note that $$\frac{d\mathcal{L}(\sigma^2)}{d\sigma^2} = -\frac{m}{2\sigma^2} +\frac{\sum_{i=1}^m(\di{y}{i} -\mu)^2}{2(\sigma^2)^2};$$ apply the chain rule, we have the new gradient 
